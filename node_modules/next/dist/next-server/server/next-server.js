            ...
            redirects = this.customRoutes.redirects.map(redirect => {
                ...
                return {
                    ...
                    fn: async (_req, res, params, _parsedUrl) => {
                        const { parsedDestination } = router_1.prepareDestination(route.destination, params);
    ...
    async renderToHTMLWithComponents(req, res, pathname, { components, query }, opts) {
        ...
        if (!isSSG) {
            ...
            if (isLikeServerless) {
                if (isDataReq) {
                    ...
                    sendPayload(res, JSON.stringify((_b = (_a = renderResult) === null || _a === void 0 ? void 0 : _a.renderOpts) === null || _b === void 0 ? void 0 : _b.pageData), 'application/json', !this.renderOpts.dev
                        ? {
                            revalidate: -1,
            ...
            if (isDataReq && isServerProps) {
                ...
                sendPayload(res, JSON.stringify(props), 'application/json', !this.renderOpts.dev
                    ? {
                        revalidate: -1,
                        private: isPreviewMode,
                    }
        ...
        if (cachedData) {
            const data = isDataReq
                ...
                : cachedData.html;
            sendPayload(res, data, isDataReq ? 'application/json' : 'text/html; charset=utf-8', cachedData.curRevalidate !== undefined && !this.renderOpts.dev
        ...
        if (!didRespond &&
            ...
            // We need to generate the fallback on-demand for development.
                ...
                if (isLikeServerless) {
                    prepareServerlessUrl(req, query);
                    html = await components.Component.renderReqToHTML(req, res);
                }
                else {
        ...
        if (!utils_2.isResSent(res)) {
            sendPayload(res, isDataReq ? JSON.stringify(pageData) : html, isDataReq ? 'application/json' : 'text/html; charset=utf-8', !this.renderOpts.dev
...
function sendPayload(res, payload, type, options) {
    ...
    if (options != null) {
        if ((_a = options) === null || _a === void 0 ? void 0 : _a.private) {
            res.setHeader('Cache-Control', `private, no-cache, no-store, max-age=0, must-revalidate`);
        }
        else if ((_b = options) === null || _b === void 0 ? void 0 : _b.revalidate) {
            res.setHeader('Cache-Control', options.revalidate < 0
                ? `no-cache, no-store, must-revalidate`
                : `s-maxage=${options.revalidate}, stale-while-revalidate`);
        }
        else if (((_c = options) === null || _c === void 0 ? void 0 : _c.revalidate) === false) {
